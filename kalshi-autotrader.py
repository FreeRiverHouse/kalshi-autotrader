...
# ── LIVE-VIABILITY: only trade markets with REAL activity ──
...
def make_trade_decision(market: MarketInfo, forecast: ForecastResult, critic: CriticResult,
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # LIVE-VIABILITY GATE — hard block, no exceptions, no paper-mode bypass.
    # A trade that can't execute in live mode MUST NOT execute in paper mode.
    # ══════════════════════════════════════════════════════════════════════════
    if market.volume == 0 and market.open_interest == 0:
        return TradeDecision(action="SKIP", edge=0.0, kelly_size=0, contracts=0,
                            price_cents=0, reason="GHOST: vol=0 + OI=0 — no counterparty, not tradable in live",
                            forecast=forecast, critic=critic)
    if market.yes_price == 50 and market.volume == 0:
        return TradeDecision(action="SKIP", edge=0.0, kelly_size=0, contracts=0,
